...
- **Пакетные ссылки + QR** — до 50 ссылок одной кнопкой; QR-код для игрока.
...
- **Турнирный режим** — событие + пакет ссылок участникам + публичная страница
  `/event/<id>` с живым статусом «проверено N из M».
- **Регулярные проверки** — watchlist игроков с интервалом (раз в N дней),
...
│   ├── index.html        ← лендинг /
│   ├── auth.html         ← вход /auth
│   ├── scans.html        ← сканы /scans (ссылки, события, watchlist, график)
...
│   ├── vendor/           ← локальные ассеты (Tailwind, Phosphor, шрифты, QR)
│   └── images/iconhtml.png
