...
Доступ к `/scans`: уровень ≥ `scans_min_level` (по умолчанию 4, меняется на
странице `/scans` уровнем 5) **или** флаг `--scan` / переключатель
«Разрешить сканы» в `/admin`.
